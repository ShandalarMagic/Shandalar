...
--color: output the "Updating sets" and "Duplicate card" lines in dark blue
...
my $n = 0;
{
  ...
  foreach my $xset (@{$xmlsets})
	...
	{
	  warn qq[No code for set "$xset->{name}"];
	  next;
	}
      ...
      unless (exists $hsets{$xset->{name}})
	{
	  exists $ignore_set_codes{$xset->{code}}
	    or warn qq[Could not find set "$xset->{name}" among $manalink_filename column headers];
	  next;
	}
...
my %newtypes =
  (
   ...
   Basic=>'2000h',
   Legendary=>'F0h',	# doesn't match subtypes.h, but does match practice in the CSVS
   Ongoing=>'2002h',
   Snow=>'2003h',
   World=>'2004h',
   Nonbasic=>'2005h',
...
foreach my $card (@{$cards})
  {
	...
	if ($printdup)
	  {
	    print STDERR qq[${color_darkblue}Duplicate card "$lastname".\tAlready in sets: ], join(", ", map { qq'"$_"' } grep { exists $add_ml{$lastname}->{$_} && $add_ml{$lastname}->{$_} ne '-' } @realsets);
	  }
	if (!$opt_update)
	  {
	    scalar @{$card->{set}} > 0 or warn qq[Card "$lastname" not assigned to any sets];
		...
		$found > 0 or warn qq[Couldn't find set for card "$lastname"'s set code "$k" despite earlier verification];
	      }
	  }
	if ($dup)
	  {
      ...
      else
	{
	  ...
	  $rules =~ s/CHAOS/|C/g;	# Planechase Chaos symbol
      ...
      unless ($opt_update && ($type =~ /^Summon / or grep { /^(UNH|UG)$/ } @{$card->{set}}))	# From Unglued/Unhinged, or old-style summon card.  Likely to just break.
	{
	  ...
	  while ($t ne '')
	      ...
	      if (!exists $newtypes{$word})
		{
		  ...
		  die qq'card "$lastname" has unknown supertype/type/subtype "$word": "$type"';
		}
	  ...
	  if ($type =~ /\bAura\b/)
	      ...
	      if ($enchant eq 'artifact creature')	# e.g. Domineer
		{
	      ...
	      if ($enchant =~ /^([a-z]+) or ([a-z]+)$/)	# e.g. Quiet Disrepair
		{
		  my $enchant1 = $1;
		  my $enchant2 = $2;
		  exists $enchant_subtypes{$enchant1} or die qq'card "$lastname" has unknown double Enchant clause: "$enchant" => "$enchant1"';
		  exists $enchant_subtypes{$enchant2} or die qq'card "$lastname" has unknown double Enchant clause: "$enchant" => "$enchant1"';
	  ...
	  #Subtype and Subtype 2 are funny.  Value 211 for None; Artifact creatures always get 44 (usually in Subtype 2); Auras get a subtype in Subtype 1 according to their Enchant clause.
	  my @sub_types;

	  $t = $type;
	  if ($t =~ s/\bAura\b// && defined $aura_subt1)
	    {
      ...
      if ($cost =~ s@ // .*$@@g)
	{
      ...
      $cost =~ s/\{([0-9]+|X+|[BUGRW]|[2PBUGRW][BUGRW])\}/|$1/g;
    ...
    $lml{'CODED CARD'} = 0;
    $lml{'Expansion'} = '8h';	# For lack of better ideas.
    ...
    $lct{'Play after Combat'} = ($equipment || $lct{'Type: Creature'}) ? 1 : 0;
...
my $correct_cols;
my $begin_updates = 0;
while (my $row = $csv->getline($manalink_in))
  {
    ++$linenum;
    if ($linenum == 1)
      {
    ...
    if (exists $add_ml{$lastname})
      {
	my $add = $add_ml{$lastname};

	if (!$updates_only)
	  {
	    say STDERR qq[Card "$lastname" is a reprint.];
	  }
	...
	if (!$updates_only)
	  {
		...
		if ($add->{$_} ne '-')
		  {
		    if ($row->[$ml_colnums{$_}] eq '-')
		      {
			++$updated_sets == 1 and print STDERR "  ${color_darkblue}Updating sets:";
			...
			print STDERR qq[ ="$_"];
		       ...
		       && $lastname !~ /^([Ss]wamp|[Ii]sland|[Ff]orest|[Mm]ountain|[Pp]lains)$/	# Omit for basic lands
		       && !/^\(/)	# Omit format-legality sets
		  {
		    ++$updated_sets == 1 and print STDERR "  ${color_darkblue}Updating sets:";
		    print STDERR qq[ <"$_"];
	    ...
	    $updated_sets > 0 and print STDERR "${color_normal}\n";
	  }
	...
	print $manalink_out $csv->string();
	$correct_cols == scalar @{$row} or warn qq[$manalink_filename row for "$lastname" had ], (scalar @{$row}), " columns, expected $correct_cols";

	delete $add_ml{$lastname};
	next;
      }
...
binmode($ctall_out);
$begin_updates = 0;
while (my $row = $csv->getline($ctall_in))
  {
    ++$linenum;
    if ($linenum == 1)
      {
	...
	if ($begin_updates && exists $add_ct{$lastname})
	  {
	    my $add = $add_ct{$lastname};
	    my @updated = ();
    ...
    if (exists $add_ct{$lastname})
      {
	delete $add_ct{$lastname};	# Don't touch this row
      }
    ...
    print $ctall_out $csv->string();
    $correct_cols == scalar @{$row} or warn qq[$ctall_filename row for "$lastname" had ], (scalar @{$row}), " columns, expected $correct_cols";
  }
