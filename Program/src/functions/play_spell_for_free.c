...
static int real_copy_spell(int player, int internal_id, int special_flags, card_instance_t* copied){
...
{
  return real_copy_spell(player, get_internal_card_id_from_csv_id(csvid), SF_NOT_CAST|SF_PLAYED_FOR_FREE, NULL);
}
...
int copy_spell_from_stack(int player, int t_player, int t_card){
	...
	return real_copy_spell(player, iid, SF_NOT_CAST | SF_PLAYED_FOR_FREE, instance);
}
...
static int play_card_in_grave_for_free_impl(int player, int t_player, int g_pos, int exile){
	...
	if( grave[g_pos] != -1 ){
		...
		if( can_legally_play_iid(player, iid) ){
			...
			if( ! real_play_spell_for_free(player, card_added, SF_PLAYED_FROM_GRAVE | (exile ? SF_EXILE_ON_RESOLUTION : 0)) ){
				obliterate_card(player, card_added);
				card_added = -1;
			}
		}
...
int play_card_in_grave_for_free_and_exile_it(int player, int t_player, int g_pos){
	return play_card_in_grave_for_free_impl(player, t_player, g_pos, 1);
}
...
int play_card_in_exile_for_free(int player, int t_player, int csvid){
	return play_card_in_exile_for_free_impl(player, player, csvid, 0, 0);
}
...
void opponent_plays_card_in_your_hand_for_free(int player, int card){
	...
	get_card_instance(1-player, card_added)->state ^= STATE_OWNED_BY_OPPONENT;

	free_spell_player = 1-player;
	free_spell_card = card_added;
	set_special_flags(1-player, card_added, SF_NOT_CAST|SF_PLAYED_FOR_FREE);
	if (put_card_on_stack(1-player, card_added, 0)){
		put_card_on_stack(1-player, card_added, 1);
		put_card_on_stack3(1-player, card_added);
	} else {
...
static int effect_may_play_card_from_exile(int player, int card, event_t event)
{
  ...
  if (event == EVENT_ACTIVATE)
	{
	  if (can_look_at_card && DIALOG(player, card, event, DLG_NO_STORAGE, DLG_NO_CANCEL, DLG_FULLCARD_CSVID(instance->display_pic_int_id),
