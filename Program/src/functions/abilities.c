...
/* result is passed in as a bitfield of the exe abilities.  The leftmost bit used is 1<<16 == 0x10000; any higher bits are removed out.
 *
 * The bits set are used in a hardcoded order to retrieve an icon from Ability.bmp.  Bits >= 1<<17, however, are used in order.
 *
 * Complex code cannot be called from this function, since it's running in a different thread from the AI.  In particular, get_ability() cannot be called, since
 * it sends an event to all cards.  The abilities are cached in what's (incorrectly) been labelled card_instance_t::regen_status.
 *
 * Similarly, we use get_displayed_card_instance() to get the version of the card instance that matches the displayed game state, rather than
 * get_card_instance() which is the one the AI is making speculative moves with. */
int get_ability_image(int player, int card, int result)
{
  ...
  /* This statement is the equivalent of !in_play(player, card), using instance instead of calling get_card_instance() internally. */
  ...
  /* 1<<17: Intimidate/Fear.  Intimidate is evergreen, and Fear formerly so.  Combining them is acceptable since they have the same sort of effect and
   * intimidate is still fairly rare (25 cards), but they should be split up once we can have more than 32 icon types. */
  ...
  // 1<<19: Unblockable.  Not actually a keyword, but a fairly common ability (124 cards) and of critical importance to assessing play.
  ...
  /* 1<<23: Double Strike.  An evergreen keyword, not terribly common (59 cards) but has high impact.
   *
   * This is often set with First Strike; KEYWORD_DOUBLE_STRIKE previously included the First Strike bit.  Hence the comparison to KEYWORD_DOUBLE_STRIKE instead
   * of just nonzero like the others, and removal of KEYWORD_FIRST_STRIKE. */
  ...
  // 1<<24: Infect.  Appears only in the Scars of Mirrodin block (67 cards), but hey, that's the current one!
  if (instance->regen_status & KEYWORD_INFECT)
	result |= 0x01000000;

  // 1<<25: Indestructible.  Evergreen, popular, and high game impact, despite its relative rarity (61 cards).
  if (sp_keywords & SP_KEYWORD_INDESTRUCTIBLE)
	result |= 0x02000000;

  /* 1<<26: Shadow.  Appears only in the Tempest and Time Spiral blocks (70 cards).  Only included due to the creatures with shadow in one of the challenge
   * gauntlet decks. */
  ...
  if (sp_keywords & SP_KEYWORD_HEXPROOF)
	/* Check of SF_HEXPROOF_OVERRIDE is deliberately missing - Glaring Spotlight doesn't actually remove hexproof, but makes creatures targettable as if they
	 * didn't have it */
	result |= 0x08000000;

  // 1<<28: Horsemanship.  Rare at 36 cards and won't be returning, but has a significant game impact when it shows up.
  if (sp_keywords & SP_KEYWORD_HORSEMANSHIP)
	result |= 0x10000000;

  // 1<<29: Defender.  Common; 207 cards mention it.
...
void disable_other_pt_setting_effects_attached_to_me(int t_player, int t_card){
	...
	for (p = 0; p <= 1; ++p){
		for (c = 0; c < active_cards_count[p]; ++c){
			...
			if (inst->internal_card_id == LEGACY_EFFECT_CUSTOM && inst->info_slot == (int)set_pt_legacy
...
void special_abilities(int player, int card, event_t event, int sp_abilities, int source_player, int source_card){
	...
	/*
	if( sp_abilities & SP_KEYWORD_PERSIST ){
		persist_granted(player, card, event);
	}
	*/
...
int attack_if_able(int player, int card, event_t event){
	if (event == EVENT_MUST_ATTACK && current_turn == player && ! forbid_attack ){
		int result = get_cost_for_propaganda_like_effect(1-player);
		if( has_phyrexian_mana(player, MANACOST_XW(BYTE0(result), BYTE1(result)), 0) ){
			...
			*fullprompt = 0;
		  else
			{
			  ...
			  if (is_what(player, card, TYPE_EFFECT))
				csvid = get_card_instance(player, card)->display_pic_int_id;
			  else
...
void granted_flanking(int s_player, int s_card, int t_player, int t_card, event_t event)
{
  ...
  if (event == EVENT_DECLARE_BLOCKERS && is_attacking(t_player, t_card) && check_for_special_ability(t_player, t_card, SP_KEYWORD_FLANKING))
	{
	  int pump = 1 + count_cards_by_id(t_player, CARD_ID_CAVALRY_MASTER);
	  if (get_id(t_player, t_card) == CARD_ID_CAVALRY_MASTER)	// doesn't give flanking to itself
		--pump;
	  ...
	  for (c = 0; c < active_cards_count[1-current_turn]; ++c)
		if (marked[1-current_turn][c])
		  alternate_legacy_text(1, s_player, pump_until_eot(s_player, s_card, 1-current_turn, c, -pump, -pump));
	}
}
