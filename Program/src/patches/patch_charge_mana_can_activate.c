...
#define INJ	"\xeb\x25\x90\x90"			/* jmp 0x42e93a */
  SEEK_AND_WRITE(f, 0x2e913);
#undef INJ
...
#define INJ	"\xeb\x10\x90"				/* jmp 0x42e9cb */
  SEEK_AND_WRITE(f, 0x2e9b9);
#undef INJ
...
#define INJ	"\xeb\x0a\x90\x90\x90\x90\x90\x90"	/* jmp 0x435c64 */
  SEEK_AND_WRITE(f, 0x35c58);
#undef INJ

  CLOSE();

  SUCCESS("patch_charge_mana_can_activate");
}
